...
class TextDiffusionModel(nn.Module):
    ...
    language model (GPT2) with a UNet for bidirectional understanding.
    """
    def __init__(
        self, 
        ...
        # Use GPT2 as the base model for text understanding
        ...
        # GPT2 layers for context understanding
    ...
    def forward(self, x, timesteps):
        ...
        # GPT2 processing for text understanding
...
class TiktokenDataset(Dataset):
    """
    Dataset using tiktoken tokenizer for more modern tokenization approach.
    """
    def __init__(self, file_path, encoding_name="cl100k_base", seq_length=512):
        self.seq_length = seq_length
        self.data = []
        self.encoding = tiktoken.get_encoding(encoding_name)
        
        print(f"Loading text data from {file_path} using tiktoken {encoding_name}...")
        with open(file_path, 'r') as f:
            for line in tqdm(f, desc="Processing dataset"):
                ...
                if len(tokens) > seq_length:
                    ...
                    tokens = tokens + [0] * (seq_length - len(tokens))  # 0 is usually the padding token
...
class GPT2Dataset(Dataset):
    """
    Dataset using GPT2 tokenizer for better compatibility with generative tasks.
    """
    def __init__(self, file_path, tokenizer, seq_length):
        ...
        print(f"Loading text data from {file_path} using GPT2 tokenizer...")
        with open(file_path, 'r') as f:
            for line in tqdm(f, desc="Processing dataset"):
                try:
                    # Try to parse as JSON if the data is structured
...
def train(args):
    ...
    # Choose tokenizer based on args
    if args.tokenizer_type == 'tiktoken':
        # No need for tokenizer instance with tiktoken, it's handled in the dataset
        dataset = TiktokenDataset(args.data_path, args.tiktoken_encoding, args.seq_length)
        # cl100k_base has ~100K tokens
        vocab_size = 100277 if args.tiktoken_encoding == "cl100k_base" else 50257  # Default to p50k_base size
    else:  # default to GPT2
        tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
        # GPT2 tokenizer doesn't have a padding token by default
        tokenizer.pad_token = tokenizer.eos_token
        dataset = GPT2Dataset(args.data_path, tokenizer, args.seq_length)
...
def generate(args):
    ...
    if args.tokenizer_type == 'tiktoken':
        ...
        mask_token_id = 50256  # Using EOS as the mask token
